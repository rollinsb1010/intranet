...
function vignereEncryptURL ($targetString)
{
    ...
    for ($i = 0; $i < count($targetArray); ++$i) {
        $encryptedURL .= $charArray[((array_search($keyArray[$i], $charArray) + array_search($targetArray[$i], $charArray)) % $numChars)];
    }
...
function vignereDecryptURL ($targetString)
{
    ...
    for ($i = 0; $i < count($targetArray); ++$i) {
        $decryptedURL .= $charArray[((($numChars + array_search($targetArray[$i], $charArray)) - array_search($keyArray[$i], $charArray)) % $numChars)];
    }
    return $decryptedURL;
}
