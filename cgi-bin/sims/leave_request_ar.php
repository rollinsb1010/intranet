...
<tr><td>
			...
			<tr><td colspan="2">
						...
						<tr><td class="body" nowrap><strong>LEAVE REQUEST</strong></td><td align="right">Leave Request ID: <?php echo $recordData['leave_request_ID'][0];?> | <a href="/staff/sims/timesheets_approve.php?Timesheet_ID=<?php echo $recordData['leave_requests::timesheet_ID'][0];?>&approver_ID=<?php echo $_SESSION['user_ID'];?>&src=intr" target="_blank" onclick="return checkTimesheet()">View this timesheet</a></td></tr>
...
<?php if($recordData['leave_requests_staff_byStaffID::c_cwp_spvsr_is_pba'][0] != '1'){ // IF THE STAFF MEMBER'S IMMEDIATE SUPERVISOR AND PRIMARY BUDGET AUTHORITY ARE NOT THE SAME PERSON ?>
												...
												<td align="center" valign="bottom"><?php if($searchData['leave_requests::signer_status_imm_spvsr'][0] == '1'){ ?><img src="/staff/sims/signatures/<?php echo $searchData['leave_requests::signer_ID_imm_spvsr'][0];?>.png"><?php } else { echo '<a href="">'.$searchData['leave_requests::signer_ID_imm_spvsr'][0].'</a>';?><?php } ?><p>
