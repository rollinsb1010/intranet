...
<table>
	...
	<thead>
		<tr>
			<th>ID</th>
			<th style="text-align:left">Pay Period</th>
			<th style="text-align:right">Total Hrs<span class="tiny">*</span></th>
			<th style="text-align:right">Work Hrs</th>
			<th style="text-align:right">Leave Hrs</th>
			<th style="text-align:left">Date/Time Submitted</th>
			<th style="text-align:left">Submitted to</th>
	...
	<tr>
	<td><?php echo $searchData['TimesheetID'][0];?></td>
	<td class="nav"><a href="/staff/sims/timesheets.php?Timesheet_ID=<?php echo $searchData['TimesheetID'][0];?>&action=view&src=menu&payperiod=<?php echo $searchData['c_PayPeriodEnd'][0];?>" title="Click here to view this timesheet."><?php echo $searchData['c_PayPeriodEnd'][0];?></a>&nbsp;&nbsp;<a style="text-decoration:none" href="timesheets_duplicate2.php?Timesheet_ID=<?php echo $searchData['TimesheetID'][0];?>" title="Create a new timesheet from this timesheet." onclick="return confirmDuplicate()"><img src="images/copy_blk_small.png" valign="middle" border="0"></a></td>
	
	<td style="text-align:right"><?php if($searchData['c_total_WkHrsReg'][0] == ''){echo '&nbsp;';}else{echo '<span title="'.$searchData['c_timesheet_hrs_email_summary'][0].'">'.$searchData['c_total_WkHrsReg'][0].'</span>';}?></td>
	<td style="text-align:right"><?php if($searchData['c_total_WkHrsRegNonLv'][0] == ''){echo '&nbsp;';}else{echo $searchData['c_total_WkHrsRegNonLv'][0];}?></td>
	<td style="text-align:right"><?php if($searchData['c_TimeSheetTotalLvHrs'][0] == ''){echo '&nbsp;';}else{echo $searchData['c_TimeSheetTotalLvHrs'][0];}?></td>
	
	<td><?php if($searchData['Signer_Timestamp_owner'][0] == ''){echo '&nbsp;';}else{echo $searchData['Signer_Timestamp_owner'][0];}?></td>
	<td><span class="tiny">SPVSR: </span><?php echo $searchData['StaffImmediateSupervisor'][0].' | <span class="tiny">PBA: </span>'.$searchData['StaffPrimaryBudgetAuthority'][0];?><?php if(($searchData['c_current_supervisor_pba_check'][0] == '1')&&($searchData['TimesheetSubmittedStatus'][0] !== 'Approved')){?><div style="float:right"><a href="timesheets_process_revised.php?action=staff_sign&row_ID=<?php echo $searchData['c_row_ID_cwp'][0];?>&mod=new_spvsr&spvsr=<?php echo $searchData['staff::immediate_supervisor_sims_user_ID'][0];?>&pba=<?php echo $searchData['staff::bgt_auth_primary_sims_user_ID'][0];?>&sisba=<?php echo $searchData['staff::c_cwp_spvsr_is_pba'][0];?>" onclick="return confirmReSign()"><img src="/common/images/bullets/exclamation.png" title="A supervisor or PBA change has occurred since this timesheet was submitted. Click this icon to re-submit this timesheet to the current supervisor and/or PBA."></div><?php }?></td>
	
	
	<?php if($searchData['TimesheetSubmittedStatus'][0]=='Approved'){ ?>
	<td style="text-align:right"><span style="color:#0000FF"><?php echo $searchData['TimesheetSubmittedStatus'][0];?></span></td>
	<td style="text-align:right"><img src="/staff/sims/images/padlock_blk_small.png" border="0"></td>
	<?php }else{ ?>
	<td style="text-align:right"><font color="red"><?php echo $searchData['TimesheetSubmittedStatus'][0];?></span></td>
	<td style="text-align:right"><a href="/staff/sims/timesheet_delete.php?row_ID=<?php echo $searchData['c_row_ID_cwp'][0];?>" onclick="return confirmDelete()" title="Click here to delete this timesheet."><img src="/staff/sims/images/trashcan.jpg" border="0"></a></td>
	<?php } ?>
	</tr>
	
	<?php } ?>
	
	<tr><td colspan="6" class="nav" style="background-color:#fbf59a"><a href="menu_timesheets.php?sortfield=c_PayPeriodEnd&displaynum=<?php echo $displaynum + 12;?>">Show more</a><?php if($displaynum > 12){?> | <a href="menu_timesheets.php?sortfield=c_PayPeriodEnd&displaynum=<?php echo $displaynum - 12;?>">Show less</a><?php }?></td><td colspan="3" align="right" style="background-color:#000000;color:#ffffff;text-align:center">Showing <?php echo $displaynum;?> records</td></tr>
	</tbody>

</table>
<span class="tiny" style="padding-left:10px">* Hover over the Total Hrs column to display a breakdown of timesheet hours.</span>
