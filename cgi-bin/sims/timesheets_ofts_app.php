...
//echo '<p>$_SESSION[signer_status_owner]: '.$_SESSION['signer_status_owner'];
...
$_SESSION['signer_fullname_owner'] = $recordData['timesheets::TimeSheetOwnerFullName'][0];
...
$_SESSION['title_owner'] = $recordData['staff::job_title'][0];
...
###############################################################################
## START: DISPLAY THE TIMESHEET IN AN HTML OR PRINT VERSION ##
###############################################################################
if($view_mode == "print"){

// START: UPDATE PRINT FLAG FOR OFTS
...
$update -> AddDBParam('print_flag','1');
$updateResult = $update -> FMEdit();
// END: UPDATE PRINT FLAG FOR OFTS

include_once('timesheets_view_print.php'); //modularized the standard timesheet view used by staff
} else {
include_once('timesheets_view_ofts.php'); //modularized the standard timesheet view used by ofts
}
#############################################################################
## END: DISPLAY THE TIMESHEET IN AN HTML OR PRINT VERSION ##
#############################################################################
