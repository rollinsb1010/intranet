...
$selected_month = $_SESSION['payperiod_selected'];
}else{
...
################################################
## START: FIND LEAVE REQUESTS FOR THIS MONTH ##
################################################
$search = new FX($serverIP,$webCompanionPort);
$search -> SetDBData('SIMS_2.fp7','leave_request_hrs_subset','all');
$search -> SetDBPassword($webPW,$webUN);
$search -> AddDBParam('c_pay_period_end_month_code',$month_code);
$search -> AddDBParam('leave_requests::signer_ID_owner',$staff);
...
################################################
## END: FIND LEAVE REQUESTS FOR THIS MONTH ##
################################################
...
<tr><td colspan="3">
			...
			<tr bgcolor="#e2eaa4" valign="top"><td class="body"><strong>Leave Calendar for <?php echo $staff;?></strong></td><td align="right">Current user: <?php echo $_SESSION['user_ID'];?> | <a href="sims_menu.php?src=intr">SIMS Home</a></td></tr>
			...
			<tr bgcolor="#e2eaa4"><td class="body" colspan=2>
							...
							<tr valign="middle" height="30"><td class="body"><h1><?= date('F', mktime(0,0,0,$selected_month_m,1,$selected_month_y)).' '.$selected_month_y; ?></h1>
								...
								<?php foreach($searchResult2['data'] as $key => $searchData2) { ?>
									<option value="ind_leave_calendar.php?staff=<?php echo $staff;?>&selected_month=<?php echo $searchData2['PayPeriodEnd_exempt'][0];?>"<?php if($searchData2['PayPeriodEnd_exempt'][0] == $pay_period_end){echo ' selected';}?>><?php echo $searchData2['PayPeriodEnd_exempt'][0];?></option>
								<?php } ?>
				...
				<?php foreach($searchResult['data'] as $key => $searchData) { 
						...
						if($searchData['c_leave_hrs_date_d'][0] == $day_num){ 
									...
									if($searchData['leave_requests::approval_status'][0] == 'Approved'){
										...
										echo '<font color="blue">'.$searchData['leave_requests::signer_ID_owner'][0].' <span class="tiny">('.$searchData['leave_hrs_time_begin'][0].'-'.$searchData['leave_hrs_time_end'][0].' | '.$searchData['c_leave_hrs_num_type'][0].')</font></span><br>';
						
									} else {
									
										//echo '<font color="red">'.$searchData['leave_requests::signer_ID_owner'][0].' - '.$searchData['c_leave_hrs_num_type'][0].'</font><br>';
										echo '<font color="red">'.$searchData['leave_requests::signer_ID_owner'][0].' <span class="tiny">('.$searchData['leave_hrs_time_begin'][0].'-'.$searchData['leave_hrs_time_end'][0].' | '.$searchData['c_leave_hrs_num_type'][0].')</font></span><br>';
