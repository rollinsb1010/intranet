...
<tr><td colspan="<?php echo $header_colspan;?>"><strong>Signatures</strong> | <font color="red">Last day to make changes to this timesheet: <strong><?php echo $_SESSION['pay_period_lockout_date'];?></strong></font></td></tr>
<tr><td colspan="<?php echo $header_colspan;?>">
	<table cellspacing=1 cellpadding=10 class="sims">
...
<?php }elseif($today_stamp <= $lockout_day_stamp){ //IF THE TIMESHEET IS NOT LOCKED ?>
			...
			<?php if($_SESSION['signer_status_owner'] != 1) { //IF THE TIMESHEET HAS NOT BEEN SIGNED YET ?>
						...
						<?php if($recordData['timesheets::RptOutsideEmplFormSigned'][0] != 1) { //IF THE REPORT OF OUTSIDE EMPLOYMENT FORM HAS NOT BEEN SIGNED YET ?>
								...
								<td align="center" class="body" bgcolor="ffffff"><a href="timesheets_rpt_out_empl.php?Timesheet_ID=<?php echo $_SESSION['timesheet_ID'];?>&action=view" onclick="return oefUnsigned()" title="Click here to sign this timesheet."><?php echo $_SESSION['signer_ID_owner'];?></a><p><span class="tiny">Staff Member</span></td>
						...
						<?php } elseif(round($AllHrsT_total,1) != round($_SESSION['payperiod_workhrs'],1) && ($_SESSION['employee_type_owner'] == 'Exempt') && ($_SESSION['new_employee_status'] == 'no') && ($_SESSION['allow_variable_timesheet_hrs'] != 'Yes')) { //IF THE TOTAL TIMESHEET HOURS DON'T MATCH THE HOURS ENTERED AND THE TIMESHEET OWNER IS EXEMPT ?>
								...
								<td align="center" class="body" bgcolor="ffffff"><a href="timesheet_process.php" onclick="return invalidHours()" title="Click here to sign this timesheet."><?php echo $_SESSION['signer_ID_owner'];?></a><p><span class="tiny">Staff Member</span></td>
						...
						<?php } elseif(($_SESSION['blank_rows_check'] > 0)&&($_SESSION['AllHrsT_total'] != $_SESSION['payperiod_workhrs'])) { //IF THERE ARE BLANK ROWS ON THE TIMESHEET AND TIMESHEET DOES NOT HAVE ALL LEAVE HRS ?>
								...
								<td align="center" class="body" bgcolor="ffffff"><a href="timesheet_process.php" onclick="return blankRows()" title="Click here to sign this timesheet."><?php echo $_SESSION['signer_ID_owner'];?></a><p><span class="tiny">Staff Member</span></td>
						...
						<?php } elseif($recordData['timesheets::c_timesheet_has_multiple_lv_type_rows'][0] == 1) { //IF THERE ARE PD LEAVE ROWS WITH DUPLICATE LV TYPES ON THE TIMESHEET ?>
								...
								<td align="center" class="body" bgcolor="ffffff"><a href="timesheet_process.php" onclick="return multipleLeavetypes()" title="Click here to sign this timesheet."><?php echo $_SESSION['signer_ID_owner'];?></a><p><span class="tiny">Staff Member</span></td>
						...
						<?php } else { //IF TIMESHEET PASSES VALIDATION ?>
								...
								<?php if($_SESSION['timesheet_owner_is_admin'] == '1'){ //IF THE TIMESHEET OWNER IS A TIME/LEAVE ADMIN ?>
									...
									<td align="center" class="body" bgcolor="ffffff"><a href="timesheets_process.php?action=staff_sign_ar&row_ID=<?php echo $recordData['timesheets::c_row_ID_cwp'][0];?>" onclick="return confirmSign()" title="Click here to sign this timesheet."><?php echo $_SESSION['signer_ID_owner'];?></a><p><span class="tiny">Staff Member</span></td>

								<?php } else { //IF THE TIMESHEET OWNER IS NOT A TIME/LEAVE ADMIN ?>

									<td align="center" class="body" bgcolor="ffffff"><a href="timesheets_process.php?action=staff_sign&row_ID=<?php echo $recordData['timesheets::c_row_ID_cwp'][0];?>" onclick="return confirmSign()" title="Click here to sign this timesheet."><?php echo $_SESSION['signer_ID_owner'];?></a><p><span class="tiny">Staff Member</span></td>
			...
			<?php } elseif($_SESSION['signer_status_owner'] == 1) { //IF THE TIMESHEET HAS BEEN SIGNED ?>
						...
						<td align="center" class="body" bgcolor="ffffff"><img src="/staff/sims/signatures/<?php echo $_SESSION['signer_ID_owner'];?>.png"><p><span class="tiny">Staff Member<br><font color="999999">[<?php echo $_SESSION['signer_timestamp_owner'];?>]</font></span></td>
			...
			<?php } else { ?>
						...
						<td align="center" class="body" bgcolor="ffffff">Error</td>
...
<!--BEGIN: IMMEDIATE SUPERVISOR / PRIMARY BUDGET AUTHORITY SIGNATURE-->									
			...
			<?php if($_SESSION['signer_pba_is_spvsr'] == 1) { //IF THE TIMESHEET OWNER'S PRIMARY BGT AUTH AND IMMEDIATE SPVSR IS THE SAME PERSON ?>									
						...
						<?php if(($_SESSION['signer_status_pba'] != 1) && ($_SESSION['current_submitted_status'] != 'Revised')) { //IF THE TIMESHEET OWNER'S PRIMARY BGT AUTH HAS NOT SIGNED THE TIMESHEET YET AND THE TIMESHEET HAS NOT BEEN REVISED ?>
							...
							<td align="center" class="body" bgcolor="ffffff" nowrap>
							
							<a href="timesheets_process.php?action=pba_sign&bgt_auth=<?php echo $_SESSION['signer_ID_pba'];?>&row_ID=<?php echo $recordData['timesheets::c_row_ID_cwp'][0];?>" 
						...
						<?php } elseif(($_SESSION['signer_status_pba'] != 1) && ($_SESSION['current_submitted_status'] == 'Revised')) { //IF THE TIMESHEET OWNER'S PRIMARY BGT AUTH HAS NOT SIGNED THE TIMESHEET YET AND THE TIMESHEET HAS BEEN REVISED ?>
							...
							<td align="center" class="body" bgcolor="ffffff" nowrap><a href="timesheets_process_revised.php?action=pba_sign&bgt_auth=<?php echo $_SESSION['signer_ID_pba'];?>&row_ID=<?php echo $recordData['timesheets::c_row_ID_cwp'][0];?>" <?php if($signature_status != 'locked'){?><?php if($_SESSION['signer_ID_pba'] == $_SESSION['user_ID']){?><?php if($_SESSION['oba_approvals_complete'] == '0'){?>onclick="return confirmSign()"<?php }else{ ?>onclick="return baMessage2()"<?php }?><?php }else{ ?>onclick="return baMessage()"<?php }?><?php }else{ ?>onclick="return arMessage()"<?php }?> title="Click here to sign this timesheet."><?php echo $_SESSION['signer_ID_pba'];?></a><p><span class="tiny">Primary Budget Authority</span></td>
						...
						<?php } elseif($_SESSION['signer_status_pba'] == 1) { //IF THE TIMESHEET OWNER'S PRIMARY BGT AUTH HAS ALREADY SIGNED THE TIMESHEET ?>
									...
									<td align="center" class="body" bgcolor="ffffff" nowrap><img src="/staff/sims/signatures/<?php echo $_SESSION['signer_ID_pba'];?>.png"><p><span class="tiny">Primary Budget Authority<br><font color="999999">[<?php echo $_SESSION['signer_timestamp_pba'];?>]</font></span></td>
						...
						<?php } else { ?>
									...
									<td align="center" class="body" bgcolor="ffffff">Error_1</td>
			...
			<?php } else { //IF THE TIMESHEET OWNER'S PRIMARY BGT AUTH AND IMMEDIATE SPVSR IS NOT THE SAME PERSON ?>
						...
						<?php } elseif(($_SESSION['signer_status_imm_spvsr'] != 1) && ($_SESSION['current_submitted_status'] == 'Revised')) { //IF THE TIMESHEET OWNER'S IMMEDIATE SUPERVISOR HAS NOT SIGNED THE TIMESHEET YET AND THE TIMESHEET HAS BEEN REVISED ?>
							...
							<td align="center" class="body" bgcolor="ffffff" nowrap><a href="timesheets_process_revised.php?action=imm_spvsr_sign&bgt_auth=<?php echo $_SESSION['signer_ID_imm_spvsr'];?>&row_ID=<?php echo $recordData['timesheets::c_row_ID_cwp'][0];?>" <?php if($signature_status != 'locked'){?><?php if($_SESSION['signer_ID_imm_spvsr'] == $_SESSION['user_ID']){?>onclick="return confirmSign()"<?php }else{ ?>onclick="return baMessage()"<?php }?><?php }else{ ?>onclick="return arMessage()"<?php }?> title="Click here to sign this timesheet."><?php echo $_SESSION['signer_ID_imm_spvsr'];?></a><p><span class="tiny">Immediate Supervisor</span></td>
						...
						<?php } elseif($_SESSION['signer_status_imm_spvsr'] == 1) { //IF THE TIMESHEET OWNER'S IMMEDIATE SUPERVISOR HAS ALREADY SIGNED THE TIMESHEET ?>
									...
									<td align="center" class="body" bgcolor="ffffff" nowrap><img src="/staff/sims/signatures/<?php echo $_SESSION['signer_ID_imm_spvsr'];?>.png"><p><span class="tiny">Immediate Supervisor<br><font color="999999">[<?php echo $_SESSION['signer_timestamp_imm_spvsr'];?>]</font></span></td>
			...
			<?php if($_SESSION['signer_status_owner'] == '1'){ 
					...
					?>
												...
												<?php if($_SESSION['current_submitted_status'] != 'Revised'){ //IF THE TIMESHEET HAS NOT BEEN REVISED
															...
															<td align="center" class="body" bgcolor="ffffff" nowrap><a href="timesheets_process.php?action=pba_sign&bgt_auth=<?php echo $current;?>&row_ID=<?php echo $recordData['timesheets::c_row_ID_cwp'][0];?>" <?php if($signature_status != 'locked'){?><?php if($current == $_SESSION['user_ID']){?><?php if(($_SESSION['oba_approvals_complete'] == '0') && ($recordData['timesheets::Signer_status_imm_spvsr'][0] == '1')){?>onclick="return confirmSign()"<?php }else{ ?>onclick="return baMessage2()"<?php }?><?php }else{ ?>onclick="return baMessage()"<?php }?><?php }else{ ?>onclick="return arMessage()"<?php }?> title="Click here to sign this timesheet."><?php echo $current;?></a><p><span class="tiny">Primary Budget Authority</span></td>
															
															
															<?php } elseif($recordData['c_hrs_approved_bgt_auth_all'][0] > '0'){ //IF THE CURRENT BUDGET AUTHORITY HAS NOT SIGNED THE TIMESHEET ?>
															
															<td align="center" class="body" bgcolor="ffffff" nowrap><a href="timesheets_process.php?action=oba_sign&bgt_auth=<?php echo $current;?>&row_ID=<?php echo $recordData['timesheets::c_row_ID_cwp'][0];?>" <?php if($signature_status != 'locked'){?><?php if($current == $_SESSION['user_ID']){?>onclick="return confirmSign()"<?php }else{ ?>onclick="return baMessage()"<?php }?><?php }else{ ?>onclick="return arMessage()"<?php }?> title="Click here to sign this timesheet."><?php echo $current;?></a><p><span class="tiny">Budget Authority</span></td>
															...
															<?php } elseif($recordData['c_hrs_approved_bgt_auth_all'][0] == '0'){ //IF THE CURRENT BUDGET AUTHORITY HAS SIGNED THE TIMESHEET ?>
																...
																<td align="center" class="body" bgcolor="ffffff" nowrap><img src="/staff/sims/signatures/<?php echo $current;?>.png"><p><span class="tiny">Budget Authority
															...
															<td align="center" class="body" bgcolor="ffffff" nowrap><a href="timesheets_process.php?action=pba_sign&bgt_auth=<?php echo $current;?>&row_ID=<?php echo $timesheet_row_ID;?>" <?php if($signature_status != 'locked'){?><?php if($current == $_SESSION['user_ID']){?><?php if($_SESSION['oba_approvals_complete'] == '0'){?>onclick="return confirmSign()"<?php }else{ ?>onclick="return baMessage2()"<?php }?><?php }else{ ?>onclick="return baMessage()"<?php }?><?php }else{ ?>onclick="return arMessage()"<?php }?> title="Click here to sign this timesheet."><?php echo $current;?></a><p><span class="tiny">Primary Budget Authority</span></td>
															...
															<?php } else { ?>
																...
																<td align="center" class="body" bgcolor="ffffff" nowrap>Error_2: <?php echo $searchResult['errorCode'];?></td>
												...
												<?php }elseif($_SESSION['current_submitted_status'] == 'Revised'){ //IF THE TIMESHEET HAS BEEN REVISED 
															...
															<td align="center" class="body" bgcolor="ffffff" nowrap><a href="timesheets_process_revised.php?action=pba_sign&bgt_auth=<?php echo $current;?>&row_ID=<?php echo $recordData['timesheets::c_row_ID_cwp'][0];?>" <?php if($signature_status != 'locked'){?><?php if($current == $_SESSION['user_ID']){?><?php if($_SESSION['oba_approvals_complete'] == '0'){?>onclick="return confirmSign()"<?php }else{ ?>onclick="return baMessage2()"<?php }?><?php }else{ ?>onclick="return baMessage()"<?php }?><?php }else{ ?>onclick="return arMessage()"<?php }?> title="Click here to sign this timesheet."><?php echo $current;?></a><p><span class="tiny">Primary Budget Authority</span></td>
															
															
															<?php } elseif($recordData['c_hrs_approved_bgt_auth_all'][0] > '0'){ //IF THE CURRENT BUDGET AUTHORITY HAS NOT SIGNED THE TIMESHEET ?>
															
															<td align="center" class="body" bgcolor="ffffff" nowrap><a href="timesheets_process_revised.php?action=oba_sign&bgt_auth=<?php echo $current;?>&row_ID=<?php echo $recordData['timesheets::c_row_ID_cwp'][0];?>" <?php if($signature_status != 'locked'){?><?php if($current == $_SESSION['user_ID']){?>onclick="return confirmSign()"<?php }else{ ?>onclick="return baMessage()"<?php }?><?php }else{ ?>onclick="return arMessage()"<?php }?> title="Click here to sign this timesheet."><?php echo $current;?></a><p><span class="tiny">Budget Authority</span></td>
															...
															<td align="center" class="body" bgcolor="ffffff" nowrap><a href="timesheets_process_revised.php?action=pba_sign&bgt_auth=<?php echo $current;?>&row_ID=<?php echo $timesheet_row_ID;?>" <?php if($signature_status != 'locked'){?><?php if($current == $_SESSION['user_ID']){?><?php if($_SESSION['oba_approvals_complete'] == '0'){?>onclick="return confirmSign()"<?php }else{ ?>onclick="return baMessage2()"<?php }?><?php }else{ ?>onclick="return baMessage()"<?php }?><?php }else{ ?>onclick="return arMessage()"<?php }?> title="Click here to sign this timesheet."><?php echo $current;?></a><p><span class="tiny">Primary Budget Authority</span></td>
															...
															<?php } else { ?>
																...
																<td align="center" class="body" bgcolor="ffffff" nowrap>Error_3: <?php echo $searchResult['errorCode'];?></td>
