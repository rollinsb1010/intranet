...
$search -> AddDBParam('c_spvsr_PBA_multikey',$_SESSION['user_ID']); //FROM STAFF THIS BUDGET AUTHORITY SUPERVISES OR IS PBA FOR
$search -> AddDBParam('sims_user_ID',$_SESSION['user_ID'],'neq'); //EXCLUDING THIS BUDGET AUTHORITY'S OWN TIMESHEET
...
$search2 -> AddDBParam('c_PayPeriodEnd',$_SESSION['payperiod_selected']); //FIND TIMESHEETS FROM THE SELECTED PAY PERIOD
...
<tr><td>
			...
			<tr><td height="33" colspan="2" scope="row"><h1>SIMS Timesheets: Budget Authority Admin</h1><hr /></td></tr>
			<tr><td class="body" nowrap><b>Budget Authority: <?php echo stripslashes($_SESSION['staff_name']);?> (<?php echo $_SESSION['workgroup'];?>)</b></td><td align="right"><a href="sims_menu.php?src=intr">SIMS Home</a></td></tr>
...
<form name="form1" method="post" action="">
						...
						<tr>
							...
							<td colspan="5" class="body" bgcolor="ebebeb" align="right">Pay Period Ending: 
								<select name="menu1" onChange="MM_jumpMenu('parent',this,0)">
									<option value="menu_timesheets_ba_admin.php?view_payperiod=<?php echo $_SESSION['payperiod_option_1'];?>"<?php if($_SESSION['payperiod_option_1'] == $_SESSION['payperiod_selected']){echo 'selected';}?>><?php echo $_SESSION['payperiod_option_1'];?></option>
									<option value="menu_timesheets_ba_admin.php?view_payperiod=<?php echo $_SESSION['payperiod_option_2'];?>"<?php if($_SESSION['payperiod_option_2'] == $_SESSION['payperiod_selected']){echo 'selected';}?>><?php echo $_SESSION['payperiod_option_2'];?></option>
									<option value="menu_timesheets_ba_admin.php?view_payperiod=<?php echo $_SESSION['payperiod_option_3'];?>"<?php if($_SESSION['payperiod_option_3'] == $_SESSION['payperiod_selected']){echo 'selected';}?>><?php echo $_SESSION['payperiod_option_3'];?></option>
									<option value="menu_timesheets_ba_admin.php?view_payperiod=<?php echo $_SESSION['payperiod_option_4'];?>"<?php if($_SESSION['payperiod_option_4'] == $_SESSION['payperiod_selected']){echo 'selected';}?>><?php echo $_SESSION['payperiod_option_4'];?></option>
									<option value="menu_timesheets_ba_admin.php?view_payperiod=<?php echo $_SESSION['payperiod_option_5'];?>"<?php if($_SESSION['payperiod_option_5'] == $_SESSION['payperiod_selected']){echo 'selected';}?>><?php echo $_SESSION['payperiod_option_5'];?></option>
									<option value="menu_timesheets_ba_admin.php?view_payperiod=<?php echo $_SESSION['payperiod_option_6'];?>"<?php if($_SESSION['payperiod_option_6'] == $_SESSION['payperiod_selected']){echo 'selected';}?>><?php echo $_SESSION['payperiod_option_6'];?></option>
								 </select>
						...
						<?php if($searchResult['foundCount'] > 0){
								...
								<td class="body" nowrap><?php echo $i.'. ';?><a href="/staff/sims/timesheets_ba_app.php?Timesheet_ID=<?php echo $searchData['TimesheetID'][0];?>&action=view&src=menu&payperiod=<?php echo $searchData['c_PayPeriodEnd'][0];?>"<?php if($searchData['TimesheetSubmittedStatus'][0] == 'Not Submitted'){echo ' onClick="return baMessage()"';}?>><?php echo stripslashes($searchData['TimeSheetName'][0]);?></a></td>
								...
								<td class="body" nowrap><?php if($searchData['approved_by_auth_rep'][0] == ''){echo '&nbsp;';}else{echo $searchData['approved_by_auth_rep'][0];}?><?php if($searchData['approved_by_auth_rep_status'][0] == '1'){echo '<em><font color="green"> - Verified</font></em>';}else{echo '<em><font color="red"> - Not verified</font></em>';}?></td>
								<td class="body"><?php if($searchData['signatures_required'][0] == ''){echo '&nbsp;';}else{echo $searchData['signatures_required'][0];}?></td>
								<td class="body" align="right" nowrap><?php if($searchData['TimesheetSubmittedStatus'][0] == 'Approved'){echo '<font color="blue">'.$searchData['TimesheetSubmittedStatus'][0].'</font>'; } else { echo '<font color="red">'.$searchData['TimesheetSubmittedStatus'][0].'</font>'; }?></td>
								</tr>
						...
						<?php if($searchResult2['foundCount'] > 0){ //IF THERE ARE OTHER TIMESHEETS FOR THIS PAY PERIOD FOR THIS BUDGET AUTHORITY TO SIGN
								...
								foreach($searchResult2['data'] as $key => $searchData2) { 
									...
									if(strpos($workgroup_list,$searchData2['TimesheetID'][0]) === false){ //IF THE CURRENT TIMESHEET IS NOT ALREADY LISTED IN THE FIRST FOUND SET OF WORKGROUP TIMESHEETS ?>
										...
										<?php }elseif(strpos($searchData2['c_signatures_needed'][0],$_SESSION['user_ID']) !== false){ ?>
