...
####################################################################
## START: FIND WORKGROUP LEAVE REQUESTS FOR THIS BUDGET AUTHORITY ##
####################################################################
$search = new FX($serverIP,$webCompanionPort);
$search -> SetDBData('SIMS_2.fp7','leave_requests_subset_2','all');
...
$search -> AddDBParam('c_spvsr_PBA_multikey',$_SESSION['user_ID']); // FROM STAFF WHO THIS BA SUPERVISES OR IS PBA FOR
$search -> AddDBParam('signer_status_owner','1'); // RESTRICT TO ONLY LEAVE REQUESTS THAT HAVE ACTUALLY BEEN SIGNED BY STAFF
...
##################################################################
## END: FIND WORKGROUP LEAVE REQUESTS FOR THIS BUDGET AUTHORITY ##
##################################################################

if(($_SESSION['user_ID'] == 'whoover')&&($show_lwop == 'yes')){
#############################################
## START: FIND LWOP LEAVE REQUESTS FOR CEO ##
#############################################
$search2 = new FX($serverIP,$webCompanionPort);
$search2 -> SetDBData('SIMS_2.fp7','leave_requests_subset_2','all');
...
$search2 -> AddDBParam('c_leave_request_contains_lwop','yes'); // FROM STAFF REQUESTING LV W/O PAY (LWOP)
$search2 -> AddDBParam('signer_status_owner','1'); // RESTRICT TO ONLY LEAVE REQUESTS THAT HAVE ACTUALLY BEEN SIGNED BY STAFF
...
$search2 -> AddSortParam('signer_status_imm_spvsr','ascend');
...
//$_SESSION['timesheet_approval_not_required'] = $recordData2['leave_requests_staff_byStaffID::no_time_leave_approval_required'][0];
###########################################
## END: FIND LWOP LEAVE REQUESTS FOR CEO ##
###########################################
}
...
<form name="form1" method="post">			
						...
						<tr>
							<td colspan="4" class="body" bgcolor="ebebeb"><?php echo $_SESSION['workgroup'];?> Leave Requests Requiring Your Approval (<?php echo $searchResult['foundCount']; ?>)<?php if(($_SESSION['user_ID'] == 'whoover')&&($show_lwop == 'yes')){ ?> | <a href="menu_leave_ba.php?view_payperiod=<?php echo $_SESSION['payperiod_selected'];?>&show_lwop=no">Hide LWOP Requests</a><?php }else{ ?> | <a href="menu_leave_ba.php?view_payperiod=<?php echo $_SESSION['payperiod_selected'];?>&show_lwop=yes">Show LWOP Requests</a><?php } ?></td>
							<td colspan="3" class="body" bgcolor="ebebeb" align="right">Pay Period Ending: 
								<select name="menu1" onChange="MM_jumpMenu('parent',this,0)">
									<option value="menu_leave_ba.php?view_payperiod=<?php echo $_SESSION['payperiod_option_1'];?>"<?php if($_SESSION['payperiod_option_1'] == $_SESSION['payperiod_selected']){echo 'selected';}?>><?php echo $_SESSION['payperiod_option_1'];?></option>
									<option value="menu_leave_ba.php?view_payperiod=<?php echo $_SESSION['payperiod_option_2'];?>"<?php if($_SESSION['payperiod_option_2'] == $_SESSION['payperiod_selected']){echo 'selected';}?>><?php echo $_SESSION['payperiod_option_2'];?></option>
									<option value="menu_leave_ba.php?view_payperiod=<?php echo $_SESSION['payperiod_option_3'];?>"<?php if($_SESSION['payperiod_option_3'] == $_SESSION['payperiod_selected']){echo 'selected';}?>><?php echo $_SESSION['payperiod_option_3'];?></option>
									<option value="menu_leave_ba.php?view_payperiod=<?php echo $_SESSION['payperiod_option_4'];?>"<?php if($_SESSION['payperiod_option_4'] == $_SESSION['payperiod_selected']){echo 'selected';}?>><?php echo $_SESSION['payperiod_option_4'];?></option>
									<option value="menu_leave_ba.php?view_payperiod=<?php echo $_SESSION['payperiod_option_5'];?>"<?php if($_SESSION['payperiod_option_5'] == $_SESSION['payperiod_selected']){echo 'selected';}?>><?php echo $_SESSION['payperiod_option_5'];?></option>
									<option value="menu_leave_ba.php?view_payperiod=<?php echo $_SESSION['payperiod_option_6'];?>"<?php if($_SESSION['payperiod_option_6'] == $_SESSION['payperiod_selected']){echo 'selected';}?>><?php echo $_SESSION['payperiod_option_6'];?></option>
									<option value="menu_leave_ba.php?view_payperiod=Other..."<?php if($_SESSION['payperiod_selected'] == 'Other...'){echo 'selected';}?>>Other...</option>
								 </select>
						...
						<?php foreach($searchResult['data'] as $key => $searchData) { ?>
							...
							<td class="body"><a href="/staff/sims/leave_request_ba.php?leave_request_ID=<?php echo $searchData['leave_request_ID'][0];?>&action=view&lwop=<?php echo $searchData['c_leave_request_contains_lwop'][0];?>&payperiod=<?php echo $searchData['pay_period_end'][0];?>"<?php if($searchData['approval_status'][0] == 'Not Submitted'){echo ' onClick="return baMessage()"';}?>><?php echo $searchData['leave_requests_staff_byStaffID::name_timesheet'][0];?></a><div style="float:right"><a href="ind_leave_calendar.php?staff=<?php echo $searchData['leave_requests_staff_byStaffID::sims_user_ID'][0];?>"><img src="/common/images/small_calendar_day.png" title="Click here to view the leave calendar for this staff member for the pay period <?php echo $_SESSION['payperiod_selected'];?>"></a></div></td>
...
<?php if(($_SESSION['user_ID'] == 'whoover')&&($show_lwop == 'yes')){ ?>

						<tr>
							<td valign="bottom" colspan="4" class="body" bgcolor="ebebeb" style="height:35px">Other Leave Requests Requiring Your Approval - LWOP (<?php echo $searchResult2['foundCount']; ?>)</td>
						...
						<?php foreach($searchResult2['data'] as $key => $searchData2) { ?>
							...
							<?php if($searchData2['signer_status_imm_spvsr'][0]=='1'){ ?>
							<td class="body" align="right"><font color="blue">Approved</font></td>
