...
function confirmSign2() { 
	var answer = confirm ("This timesheet has not been verified by your workgroup's Timesheet Admin. Sign anyway?")
	if (!answer) {
	return false;
	}
}
...
function baMessage3() { 
	alert ("Immediate Supervisor must sign this timesheet before other budget authorities.")
	return false;
}
...
<tr><td colspan="3">
			...
			<tr><td colspan="2">
				<p class="alert_small"><b>Budget Authority: <?php echo $_SESSION['staff_name'];?> (<?php echo $_SESSION['workgroup'];?>)</b> - To approve this timesheet, click the appropriate signature box below. | <a href="timesheets_rpt_out_empl_print.php?Timesheet_ID=<?php echo $_SESSION['timesheet_ID'];?>&action=print" target="_blank">OE Form</a> | <input type=button value="Close Timesheet" onClick="history.back()"></p>
			</td></tr>
			
			<?php } elseif(($action == 'view') && ($recordData['timesheets::TimesheetSubmittedStatus'][0] == 'Approved')){ ?>
			
			<tr><td colspan="2">
				<p class="alert_small"><b>Budget Authority: <?php echo $_SESSION['staff_name'];?> (<?php echo $_SESSION['workgroup'];?>)</b> - This timesheet has been approved. <img src="/staff/sims/images/green_check.png"> | <a href="timesheets_rpt_out_empl_print.php?Timesheet_ID=<?php echo $_SESSION['timesheet_ID'];?>&action=print" target="_blank">OE Form</a> | <input type=button value="Close Timesheet" onClick="history.back()"></p>
			</td></tr>			
			
			<?php } elseif($action == 'approve'){ ?>
			
			<tr><td colspan="2">
				<p class="alert_small"><b>Budget Authority: <?php echo $_SESSION['staff_name'];?> (<?php echo $_SESSION['workgroup'];?>)</b> - You have successfully approved this timesheet. <img src="/staff/sims/images/green_check.png"> | <a href="timesheets_rpt_out_empl_print.php?Timesheet_ID=<?php echo $_SESSION['timesheet_ID'];?>&action=print" target="_blank">OE Form</a> | <a href="/staff/sims/menu_timesheets_ba_admin.php?view_payperiod=<?php echo $_SESSION['payperiod_selected'];?>">Close Timesheet</a></p>
			</td></tr>			
			...
			<tr><td class="body"><?php if($recordData['timesheets::TimesheetSubmittedStatus'][0] == 'Revised'){ ?>&nbsp;<strong><i><font color="#FC0000">REVISED ROWS IN RED</font></i></strong><?php }else{ ?>&nbsp;<i>NOTE: Budget codes for <strong><?php echo $_SESSION['user_ID'];?></strong> highlighted in gold.</i><?php } ?></td><td align="right">Timesheet ID: <?php echo $_SESSION['timesheet_ID'];?></td></tr>
...
<!--BEGIN FIRST SECTION: REGULAR HOURS BY BUDGET CODE-->	
			...
			<tr><td class="body" colspan=2>
							...
							<table cellspacing=0 width="100%" class="sims">
							<tr><td colspan="<?php echo $header_colspan;?>"><strong>Regular Hours by Budget Code:</strong></td></tr>
									...
									$i++;
									?>
									<tr <?php  if($searchData['TimeRevisedStatus'][0] == '1'){echo'bgcolor="#FA93A1"';} elseif($searchData['BudgetAuthorityLocal'][0] == $_SESSION['user_ID']){?>bgcolor="#F0E9B9"<?php }?>>
...
<?php if($_SESSION['employee_type_owner'] == 'Non-exempt') { ?>
							...
							<!--table cellspacing=0 cellpadding=1 border=1 bordercolor="cccccc" width="100%" class="body"-->
									...
									<tr <?php  if($searchData5['TimeRevisedStatus'][0] == '1'){echo'bgcolor="#FA93A1"';} elseif($searchData5['BudgetAuthorityLocal'][0] == $_SESSION['user_ID']){?>bgcolor="#F0E9B9"<?php }?>>
