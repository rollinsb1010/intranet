...
$old_timesheet_ID = $_GET['Timesheet_ID'];
...
$search -> AddDBParam('-script', 'duplicate_timesheet_web2');
...
##############################################################################
## START: CREATE DUPLICATE BUDGET CODE ROWS FOR NEW TIMESHEET
##############################################################################

// FIND BUDGET CODES USED ON DUPLICATED TIMESHEET

$search8 = new FX($serverIP,$webCompanionPort);
$search8 -> SetDBData('SIMS_2.fp7','time_hrs');
$search8 -> SetDBPassword($webPW,$webUN);
$search8 -> AddDBParam('Timesheet_ID','=='.$old_timesheet_ID);
$search8 -> AddDBParam('HrsType','WkHrsReg');
$search8 -> AddDBParam('c_budget_code_active_status','Active');

$searchResult8 = $search8 -> FMFind();

//echo $searchResult['errorCode'];
//echo $searchResult['foundCount'];
//$recordData8 = current($searchResult8['data']);

// LOOP THROUGH PREVIOUS BUDGET CODES, CREATING A NEW TIME_HRS ROW FOR EACH REG_HRS BUDGET CODE
foreach($searchResult8['data'] as $key => $searchData8) { 
...
$newrecord4 -> AddDBParam('BudgetCode',$searchData8['BudgetCode'][0]);
$newrecord4 -> AddDBParam('HrsType','WkHrsReg');


$newrecordResult4 = $newrecord4 -> FMNew();

//echo  '<p>errorCode: '.$newrecordResult4['errorCode'];
//echo  '<p>foundCount: '.$newrecordResult4['foundCount'];
...
##############################################################################
## END: CREATE DUPLICATE BUDGET CODE ROWS FOR NEW TIMESHEET
##############################################################################
