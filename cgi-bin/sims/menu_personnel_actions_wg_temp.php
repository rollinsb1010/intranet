...
<tr><td colspan="3">
			...
			<tr bgcolor="#e2eaa4"><td class="body"><strong>Workgroup <?php if($query == 'former_staff'){?>Former <?php }?>Staff (Temp)</strong> | <?php echo $searchResult['foundCount'];?> records found. | <?php if($query == 'former_staff'){?><a href="menu_personnel_actions_wg_temp.php?action=show_all">Show current temp staff</a><?php }else{?><a href="menu_personnel_actions_wg_temp.php?action=show_all&query=former_staff">Show former temp staff</a><?php }?></td><td align="right">Current user: <?php echo $_SESSION['user_ID'];?> | <a href="sims_menu.php?src=intr">SIMS Home</a></td></tr>
...
<tr><td>
			...
			<tr><td height="33" colspan="2" scope="row"><h1>SIMS - Temporary Personnel Actions</h1><hr /></td></tr>
			<tr><td class="body" nowrap><b><?php echo $fullname;?> (<?php echo $unit;?>)</b></td><td align="right">Current user: <?php echo $_SESSION['user_ID'];?> | <a href="menu_personnel_actions_wg_temp.php?action=new&id=<?php echo $recordData2['staff_ID'][0];?>" title="Create new personnel action for this staff member">New temporary personnel action</a> | <a href="menu_personnel_actions_wg_temp.php?action=show_all" title="Return to workgroup Personnel Actions.">Workgroup staff</a> | <a href="sims_menu.php?src=intr" title="Return to your SIMS home screen.">SIMS Home</a></td></tr>
			...
			<tr><td colspan="2">
						...
						<?php if($searchResult['foundCount'] > 0) { ?>
							...
							<td class="body" style="vertical-align:text-top"><a href="/staff/sims/menu_personnel_actions_wg_temp.php?record_ID=<?php echo $searchData['record_ID'][0];?>&action=show_action" title="Click here to view this personnel action." target="_blank"><?php echo $searchData['action_effective_date'][0];?></a></td>
...
if($_REQUEST['mod'] == 'submit_new'){
	...
	##################################################
	## START: CREATE NEW TEMP PERSONNEL ACTION ##
	##################################################
	...
	$newrecord -> AddDBParam('document_type','Temporary');
	
	if($_REQUEST['staff_address'] !== ''){
	...
	$newrecord -> AddDBParam('assign_to_hourly_rate',$_REQUEST['assign_to_hourly_rate']);
	...
	##################################################
	## END: CREATE NEW TEMP PERSONNEL ACTION ##
	##################################################
	...
	$subject = 'SIMS: TEMP PERSONNEL ACTION RECEIVED';
	$message = 
	'Unit Budget Authority:'."\n\n".
	
	'A new Temporary Personnel Action has been received by SIMS for staff member ('.$newrecordData['staff::c_full_name_first_last'][0].') and requires your approval.'."\n\n".
	...
	'To review and approve this temporary personnel action, click here:'."\n".
	'http://www.sedl.org/staff/sims/menu_personnel_actions_wg_temp.php?action=show_action&record_ID='.$newrecordData['record_ID'][0]."\n\n".
...
if($_REQUEST['mod'] == 'pba'){
	...
	'A new Temporary Personnel Action has been received by SIMS for staff member ('.$updateData['staff::c_full_name_first_last'][0].') and requires your approval.'."\n\n".
	...
	'To review and approve this temporary personnel action, click here:'."\n".
	'http://www.sedl.org/staff/sims/menu_personnel_actions_wg_temp.php?action=show_action&record_ID='.$updateData['record_ID'][0]."\n\n".
...
if($_REQUEST['mod'] == 'ceo'){
	...
	//$to = 'ewaters@sedl.org';
	$to = $updateData['signer_ID_hr'][0].'@sedl.org';
	$subject = 'SIMS: TEMP PERSONNEL ACTION APPROVED';
	$message = 
	'HR Generalist:'."\n\n".
	
	'A new Temporary Personnel Action has been approved by the CEO for staff member ('.$updateData['staff::c_full_name_first_last'][0].').'."\n\n".
	...
	'This temporary personnel action approval has not been released to the staff member. To review, print, and/or release this temporary personnel action, click here:'."\n".
	
	'http://www.sedl.org/staff/sims/menu_personnel_actions_temp_admin.php?action=show_action&record_ID='.$updateData['record_ID'][0]."\n\n".
...
function confirmSign() { 
	var answer = confirm ("Sign this temporary personnel action?")
	if (!answer) {
	return false;
	}
}
...
function preventSignPBA() { 
	alert ("This signature box is reserved for the Unit Budget Authority. To sign this temporary personnel action, click the box with your ID.")
	return false;
}
...
function preventSignCFO() { 
	alert ("This signature box is reserved for the CFO. To sign this temporary personnel action, click the box with your ID.")
	return false;
}

function preventSignCEO() { 
	alert ("This signature box is reserved for the CEO. To sign this temporary personnel action, click the box with your ID.")
	return false;
}
...
<tr><td colspan="2" style="vertical-align:text-top;padding:6px;border:0px"><div class="alert_small">New Personnel Action successfully created - notification sent to unit budget authority. | <a href="menu_personnel_actions_wg_temp.php?action=show_1&staff_ID=<?php echo $recordData['staff_ID'][0];?>">Close document</a></div></td></tr>
<?php }?>
...
<tr><td width="50%" style="vertical-align:text-top;padding:6px;border-top:1px solid #333333; border-right:solid 1px #333333">
	...
	<strong>3. FISCAL REVIEW</strong><br><span style="margin-left:15px"><?php if($recordData['sign_status_cfo'][0] !== '1'){?><a href="menu_personnel_actions_wg_temp.php?action=show_action&mod=cfo&record_ID=<?php echo $recordData['record_ID'][0];?>&id=<?php echo $recordData['c_row_ID'][0];?>"  <?php if($recordData['signer_ID_cfo'][0] == $_SESSION['user_ID']){echo 'onclick="return confirmSign()"';}else{echo 'onclick="return preventSignCFO()"';}?>><?php echo $recordData['signer_ID_cfo'][0];?></a><?php }else{ ?><img src="signatures/<?php echo $recordData['signer_ID_cfo'][0];?>.png"><br><span class="tiny" style="color:#666666;margin-left:15px;padding:2px">[<?php echo $recordData['sign_timestamp_cfo'][0];?>]</span><?php }?></span><br><span class="tiny" style="margin-left:15px;border-top:1px dotted #666666"><strong>CHIEF FINANCIAL OFFICER</strong></span></td></td>
	
	<td width="50%" style="vertical-align:text-top;padding:6px;border-top:1px solid #333333">

	<strong>4. APPROVAL</strong><br><span style="margin-left:15px"><?php if($recordData['sign_status_ceo'][0] !== '1'){?><a href="menu_personnel_actions_wg_temp.php?action=show_action&mod=ceo&record_ID=<?php echo $recordData['record_ID'][0];?>&id=<?php echo $recordData['c_row_ID'][0];?>"  <?php if($recordData['signer_ID_ceo'][0] == $_SESSION['user_ID']){echo 'onclick="return confirmSign()"';}else{echo 'onclick="return preventSignCEO()"';}?>><?php echo $recordData['signer_ID_ceo'][0];?></a><?php }else{ ?><img src="signatures/<?php echo $recordData['signer_ID_ceo'][0];?>.png"><br><span class="tiny" style="color:#666666;margin-left:15px;padding:2px">[<?php echo $recordData['sign_timestamp_ceo'][0];?>]</span><?php }?></span><br><span class="tiny" style="margin-left:15px;border-top:1px dotted #666666"><strong>PRESIDENT & CEO</strong></span></td></tr>
</table>
...
<tr><td width="50%" style="vertical-align:text-top;padding:6px;border-top:1px solid #333333; border-right:solid 1px #333333">
	...
	<strong>NATURE OF ACTION:</strong><br>
		...
		<?php foreach($v1Result['valueLists']['personnel_action_types_temp'] as $key => $value) { ?>
...
<tr><td width="50%" style="vertical-align:text-top;padding:6px;border-top:1px solid #333333; border-right:solid 1px #333333">
	...
	<strong>HOURLY RATE:</strong><br>
	$<input type="text" name="assign_to_hourly_rate" size="15">/hr.<br>(Ex: 25.50 - no commas or currency symbols)
...
Error | <a href="menu_personnel_actions_wg_temp.php?action=show_all" title="Return to Temporary Personnel Actions screen.">Return to Temporary Personnel Actions Admin</a>
