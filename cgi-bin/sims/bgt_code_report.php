...
function checkFields() { 
	...
	// Budget Code - Fund Year
		if ((document.form2.budget_code.value !=="")&&(document.form2.fund_year.value !=="")) {
			alert("Select either a budget code or fund year for this report - not both.");
			document.form2.budget_code.focus();
			return false;	}
}
...
function checkFields2() { 
	...
	// Summarize By
		...
		if (!document.form3.summarize_by[0].checked && !document.form3.summarize_by[1].checked  && !document.form3.summarize_by[2].checked) {
			...
			<tr><td class="body" colspan=2>
							...
							<tr><td class="body" valign="top" width="100%">
								...
								<tr><td valign="top">
											...
											<table cellspacing="0" cellpadding="5" border="0" width="100%">
											<tr><td colspan="2"><strong>Budget Code/Fund Year Summary by Date Range:</strong></td></tr>
...
<hr>
											...
											<table cellspacing="0" cellpadding="5" border="0" width="100%">
											<tr><td colspan="2"><strong>Staff Time Summary by Budget Code/Task/PTO:</strong></td></tr>
											...
											<tr valign="top"><td align="right"><font color="666666">Summarize By:</font></td><td>
												...
												<input type="radio" name="summarize_by" value="task"> Task/Budget Code<br>
												<input type="radio" name="summarize_by" value="pto"> Paid Time Off (PTO)
...
if($summarize_by == 'pto'){
$search -> AddDBParam('HrsType','PdLv');
$search -> AddSortParam('timesheets::c_PayPeriodEnd','ascend');
} else {
...
if($summarize_by == 'task'){
 ...
 } elseif($summarize_by == 'fund_year') { // IF $summarize_by == 'fund_year'
 ...
 <?php } else {  //IF $summarize_by == 'pto' 
		...
		$num_months = count($month_name);
		$data_table_colspan = $num_months + 2;

		//echo '<p>$num_months: '.$num_months;
		//echo '<p>$total_hrs: ';
		//print_r($total_hrs);
		//echo '<P>$month_name: ';
		//print_r($month_name);
		...
		#################################
		$rand = rand();
		$xml_header = "<chart caption='Staff PTO Summary by Month: $begin_date - $end_date' subcaption='Staff: $staff_id' xAxisName='Month' yAxisName='Paid Leave Hours Charged' numberPrefix=''>
		";
		//$file_name = "bgt_cd_summary.xml";
		$file_name = 'bgt_cd_summary.xml';
		...
		<?php $_SESSION['budget_code_deleted'] = ''; }?>
					...
					<tr bgcolor="#e2eaa4"><td class="body"><strong>Staff PTO Summary by Month: <?php echo $searchResult['foundCount'];?> records found.</td><td align="right">Current user: <?php echo $_SESSION['user_ID'];?> <a href="bgt_code_report.php?action=new">New Report</a> | <a href="sims_menu.php?src=intr">SIMS Home</a></td></tr>
					...
					<tr><td class="body" colspan=2>
									...
									<table cellspacing="0" cellpadding="4" width="100%" class="sims">
									<tr bgcolor="#ebebeb"><td class="body" nowrap>
									Paid Leave Hours charged by <strong><?php echo $staff_id;?></strong> from <strong><?php echo $begin_date;?></strong> to <strong><?php echo $end_date;?></strong></td></tr>
									<tr><td>
									...
									<tr bgcolor="#ebebeb"><td class="body" nowrap colspan="<?php echo $data_table_colspan;?>">
									Data Table</td></tr>
									<tr><td nowrap>Month</td>
									
		
									<?php for($j=0;$j<=$num_months;$j++) { 
									
										if(isset($month_name[$j])){ echo '<td>'.$month_name[$j].'</td>'; }
									...
									<?php for($j=0;$j<=$num_months;$j++) { 
										...
										if(isset($month_name[$j])){ echo '<td>'.$total_hrs[$j].'</td>'; }
