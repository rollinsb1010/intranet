...
if($approve == 'spvsr') { // IF THE SUPERVISOR APPROVED THE TIMESHEET
...
$update -> AddDBParam('signer_timestamp_imm_spvsr_trigger',$trigger);
...
$lwop = $recordData['c_leave_request_contains_lwop'][0];
...
if($updateResult['errorCode'] == '0'){
	...
	$newrecord -> AddDBParam('user',$recordData['signer_ID_imm_spvsr'][0]);
	$newrecord -> AddDBParam('action','SIGN_LEAVE_REQUEST_SPVSR');
...
$_SESSION['leave_request_signed_imm_spvsr'] = '1';
		...
		$subject = $recordData['leave_requests_staff_byStaffID::c_full_name_first_last'][0].' has submitted a '.$revised_flag.'leave request requiring your approval';
		$message = 
		'Dear '.$recordData['leave_requests_staff_byStaffID::c_fullname_pba'][0].','."\n\n".
		
		'A '.$revised_flag.'leave request for '.$recordData['leave_requests_staff_byStaffID::c_full_name_first_last'][0].' has been submitted for the pay period ending '.$recordData['pay_period_end'][0].'.'."\n\n".
...
$_SESSION['leave_request_signed_imm_spvsr'] = '2';
...
$lwop = $recordData['leave_requests::c_leave_request_contains_lwop'][0];
...
function spvsrMessage() { 
	var answer2 = confirm ("This space reserved for Primary Budget Authority.")
	return false;
}
...
<tr><td>
			...
			<?php if($recordData['leave_requests::signer_status_imm_spvsr'][0] != '1'){ ?>
			
			<tr><td colspan="2">
				<p class="alert_small"><b>Supervisor: <?php echo $_SESSION['staff_name'];?> (<?php echo $_SESSION['workgroup'];?>)</b> - To approve this leave request, click the appropriate signature box below. | <input type=button value="Close Leave Request" onClick="history.back()"></p>
			</td></tr>
			
			<?php } elseif($recordData['leave_requests::signer_status_imm_spvsr'][0] == '1'){ ?>
			
			<tr><td colspan="2">
				<p class="alert_small"><b>Supervisor: <?php echo $_SESSION['staff_name'];?> (<?php echo $_SESSION['workgroup'];?>)</b> - You have successfully approved this leave request. <img src="/staff/sims/images/green_check.png"> | <?php if($_SESSION['menu_type'] == 'spvsr_admin'){?><a href="/staff/sims/menu_leave_spvsr.php?view_payperiod=<?php echo $_SESSION['payperiod_selected'];?>">Close Leave Request</a><?php }else{?><a href="/staff/sims/menu_leave_ba.php?view_payperiod=<?php echo $_SESSION['payperiod_selected'];?>">Close Leave Request</a><?php }?></p>
			</td></tr>			
			...
			<tr><td colspan="2">
						...
						<tr><td colspan="2">
							...
							<table cellpadding="8" cellspacing="0" border="1" bordercolor="#ebebeb" width="100%" class="sims">
								<tr bgcolor="#ebebeb"><td class="body" nowrap>Leave Type</td><td class="body">Date</td><td class="body">From</td><td class="body">To</td><td class="body" align="right">Hours</td><td class="body">Description (optional)</td></tr>

								<?php foreach($searchResult['data'] as $key => $searchData) { ?>
								

										<tr class="body"><td nowrap><?php echo $searchData['leave_hrs_type'][0];?><?php if($searchData['c_lv_hrs_requires_documentation'][0] == '1'){echo '*';}?></td><td nowrap><?php echo $searchData['leave_hrs_date'][0];?> <span class="tiny">(<?php echo strtoupper($searchData['c_leave_hrs_day_name'][0]);?>)</span></td><td nowrap><?php echo $searchData['leave_hrs_time_begin'][0];?></td><td nowrap><?php echo $searchData['leave_hrs_time_end'][0];?></td><td nowrap align="right"><?php echo $searchData['leave_num_hrs'][0];?></td><td width="100%"><?php echo $searchData['leave_hrs_description'][0];?></td></tr>
...
<?php if($recordData['leave_requests_staff_byStaffID::c_cwp_spvsr_is_pba'][0] != '1'){ // IF THE STAFF MEMBER'S IMMEDIATE SUPERVISOR AND PRIMARY BUDGET AUTHORITY ARE NOT THE SAME PERSON ?>
												...
												<td align="center" valign="bottom"><?php if($searchData['leave_requests::signer_status_imm_spvsr'][0] == '1'){ // IF THE IMMEDIATE SUPERVISOR HAS SIGNED THE LEAVE REQUEST ?>
												
													<img src="/staff/sims/signatures/<?php echo $searchData['leave_requests::signer_ID_imm_spvsr'][0];?>.png"><?php } else { // IF THE IMMEDIATE SUPERVISOR HAS NOT SIGNED THE LEAVE REQUEST
													
													echo '<a href="http://www.sedl.org/staff/sims/leave_request_spvsr.php?action=view&approve=spvsr&leave_request_ID='.$recordData['leave_request_ID'][0].'&leave_request_row_ID='.$searchData['leave_requests::c_row_ID_cwp'][0].'&status='.$recordData['leave_requests::approval_status'][0].'" onClick="return confirmSign()">'.$searchData['leave_requests::signer_ID_imm_spvsr'][0].'</a>';?><?php } ?><p>
												...
												<?php } else {  // IF THE IMMEDIATE SUPERVISOR HAS SIGNED THE LEAVE REQUEST BUT THE PBA HAS NOT SIGNED THE LEAVE REQUEST
													...
													echo '<a href="http://www.sedl.org/staff/sims/leave_request_ba.php?action=view&approve=ba&leave_request_ID='.$recordData['leave_request_ID'][0].'&leave_request_row_ID='.$searchData['leave_requests::c_row_ID_cwp'][0].'" ';?><?php if($_SESSION['user_ID'] == $searchData['leave_requests::signer_ID_pba'][0]){?>onClick="return confirmSign()"<?php }else{?>onClick="return spvsrMessage()"<?php }?><?php echo'>'.$searchData['leave_requests::signer_ID_pba'][0].'</a>';?>
...
<?php } else {  // IF THE STAFF MEMBER'S IMMEDIATE SUPERVISOR AND PRIMARY BUDGET AUTHORITY ARE THE SAME PERSON ?>
												...
												<td align="center" valign="bottom"><?php if($searchData['leave_requests::signer_status_pba'][0] == '1'){ ?><img src="/staff/sims/signatures/<?php echo $searchData['leave_requests::signer_ID_pba'][0];?>.png"><?php } else { echo '<a href="http://www.sedl.org/staff/sims/leave_request_ba.php?action=view&lwop='.$lwop.'&approve=ba&leave_request_ID='.$recordData['leave_request_ID'][0].'&leave_request_row_ID='.$searchData['leave_requests::c_row_ID_cwp'][0].'" onClick="return confirmSign()">'.$searchData['leave_requests::signer_ID_pba'][0].'</a>';?><?php } ?><p>
												<span class="tiny">Primary Budget Authority<br><font color="999999">[<?php echo $searchData['leave_requests::signer_timestamp_pba'][0];?>]</font></span></td>
