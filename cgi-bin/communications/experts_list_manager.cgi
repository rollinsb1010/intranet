...
#
# This script is used by Communications to manage the home page list of Experts
...
########################################
my $item_label = "Expert Listing";
my $site_label = "SEDL Expert List Manager";
...
my $side_nav_menu_code = "";
   $side_nav_menu_code =  &intranetcommoncode::get_side_nav_menu_code("420"); # 420 is the PID for this page in the intranet database
...
print<<EOM;      
<p>
Welcome to the $site_label. This database is used by Communications staff (Joni, Laura, Brian, Shaila) 
...
<FORM ACTION="experts_list_manager.cgi" METHOD="POST">
...
if ($location eq 'process_delete_item') {
	...
	## END: BACKSLASH VARIABLES FOR DB
		...
		## DELETE THE PAGES
		my $command_delete_item = "DELETE from experts WHERE expert_id = '$show_record'";
...
#################################################################################
	my $new_expert_name = $query->param("new_expert_name");
	my $new_expert_description = $query->param("new_expert_description");
	my $new_expert_title = $query->param("new_expert_title");
	my $new_expert_photo = $query->param("new_expert_photo");
	my $new_expert_link = $query->param("new_expert_link");
	my $new_last_updated = $query->param("new_last_updated");
	my $new_active = $query->param("new_active");
	my $new_expert_pd_session = $query->param("new_expert_pd_session");

	## START: CHECK FOR DATA COPLETENESS
	if ($location eq 'process_add_item') {
		if ($new_expert_name eq '') {
			$error_message .= "The Expert Name is missing. Please try again.";
			$location = "add_item";
		} # END IF
	} # END IF
...
if ($location eq 'process_add_item') {
	...
	$new_expert_name = &cleanformysql($new_expert_name);
	$new_expert_description = &cleanformysql($new_expert_description);
	$new_expert_title = &cleanformysql($new_expert_title);
	$new_expert_photo = &cleanformysql($new_expert_photo);
	$new_expert_link = &cleanformysql($new_expert_link);
	$new_last_updated = &cleanformysql($new_last_updated);
	$new_active = &cleanformysql($new_active);
	$new_expert_pd_session = &cleanformysql($new_expert_pd_session);
	## END: BACKSLASH VARIABLES FOR DB


	# CHECK TO ENSURE ITEM NOT ALREADY SUBMITTED
	my $already_exists = "no";
		my $command = "select * from experts ";
#			if ($show_record ne '') {
				$command .= "WHERE expert_id = '$show_record'";
#			}
		$dsn = "DBI:mysql:database=corp;host=localhost";
		my $dbh = DBI->connect($dsn, "corpuser", "public");
		my $sth = $dbh->prepare($command) or die "Couldn't prepare statement: " . $dbh->errstr;
...
#			$feedback_message .= "<p>COMMAND CHECK DUPLICATES: $command</p><p>MATCHES: $num_matches_code</p>";
		...
		if ($already_exists eq 'yes') {
			## DO THE EDIT
			my $command_update = "UPDATE experts 
										SET expert_name='$new_expert_name', expert_description='$new_expert_description', expert_title='$new_expert_title', expert_photo='$new_expert_photo', expert_link='$new_expert_link', last_updated='$timestamp', active='$new_active', expert_pd_session='$new_expert_pd_session'
										WHERE expert_id ='$show_record'";
			my $dbh = DBI->connect($dsn, "corpuser", "public");
			...
			my $command_insert = "INSERT INTO experts VALUES ('', '$new_expert_name', '$new_expert_description', '$new_expert_title', '$new_expert_photo', '$new_expert_link', '$timestamp', '$new_active', '$new_expert_pd_session')";
			my $dbh = DBI->connect($dsn, "corpuser", "public");
...
if ($location eq 'add_item') {
	...
	my $expert_id = "";
	my $expert_name = "";
	my $expert_description = "";
	my $expert_title = "";
	my $expert_photo = "";
	my $expert_link = "";
	my $last_updated = "";
	my $active = "";
	my $expert_pd_session = "";
	
	if ($show_record ne '') {
		$page_title = "Save Edits to this $item_label";

		# SELCT EXISTING INFO FROM DB
		my $command = "select * from experts WHERE expert_id = '$show_record'";
		$dsn = "DBI:mysql:database=corp;host=localhost";
		my $dbh = DBI->connect($dsn, "corpuser", "public");
		my $sth = $dbh->prepare($command) or die "Couldn't prepare statement: " . $dbh->errstr;
		$sth->execute;
		...
		while (my @arr = $sth->fetchrow) {
			($expert_id, $expert_name, $expert_description, $expert_title, $expert_photo, $expert_link, $last_updated, $active, $expert_pd_session) = @arr;
		} # END DB QUERY LOOP
		...
		$expert_name = &cleanaccents2html($expert_name);
		$expert_description = &cleanaccents2html($expert_description);
		$last_updated = &convert_timestamp_2pretty_w_date($last_updated);
...
<h1><A HREF="experts_list_manager.cgi">$site_label</A><br>
$page_title</h1>
...
print<<EOM;      
<FORM ACTION="experts_list_manager.cgi" METHOD="POST" name="form2" id="form2">

<TABLE border="1" cellpadding="2" cellspacing="0" width="100%">
<tr><td valign="top"><strong>Expert Name</strong></td>
	<td valign="top"><textarea name="new_expert_name" rows="8" cols=70>$expert_name</textarea>
	</td></tr>
<tr><td valign="top"><strong>Active</strong></td>
	<td valign="top">
EOM
&print_yes_no_menu("new_active", $active);
print<<EOM;
	</td></tr>

<tr><td valign="top"><strong>Title</strong></td>
	<td valign="top">
		<input type="text" name="new_expert_title" id="new_expert_title" value="$expert_title" size="50">
	</td></tr>

<tr><td valign="top"><strong>PD Session for Heading</strong></td>
	<td valign="top">
		<input type="text" name="new_expert_pd_session" id="new_expert_pd_session" value="$expert_pd_session" size="50">
	</td></tr>

<tr><td valign="top"><strong>Description</strong><p>Use the "HTML" button to edit the HTML source code.</p></td>
	<td valign="top"><textarea name="new_expert_description" rows="12" cols=70>$expert_description</textarea>
		<br>(Note: Embed links to actionable items like a CPL page in the description.)
	</td></tr>

<tr><td valign="top"><strong>Photo URL</strong><br>(will be resized to 102px tall on home page)</td>
	<td valign="top">
		<input type="text" name="new_expert_photo" id="new_expert_title" value="$expert_photo" size="80"><br>
		(URL example: http://www.sedl.org/images/people/rjarvis.jpg)
EOM
	if ($expert_photo ne '') {
		print "<br><img src=\"$expert_photo\" height=\"102\">";
	}
print<<EOM;
	</td></tr>


<tr><td valign="top"><strong>Expert Link</strong> (for the "read more" button)</td>
	<td valign="top">		<input type="text" name="new_expert_link" id="new_expert_link" value="$expert_link" size="80"><br>
		(URL example: http://www.sedl.org/pubs/catalog/authors/rjarvis.html)
	</td></tr>
<tr><td valign="top"><strong>Last Updated</strong></td>
	<td valign="top">$last_updated</td></tr>
...
print<<EOM;
	...
	<td valign="top" align="right">
		(Click here to <A HREF="experts_list_manager.cgi?location=logout">logout</A>)
		<P>
	</td></tr>
</table>
...
$htmlhead
<table Cellpadding="0" cellspacing="0" border="0" width="100%">
<tr><td><h1><A HREF="experts_list_manager.cgi">$site_label</A>
...
my $command = "select * from experts";
	$command .= " order by expert_name" if $sortby eq '';
	$command .= " order by last_updated DESC, expert_name" if $sortby eq 'lastupdated';
	$command .= " order by active, expert_name" if $sortby eq 'active';
...
my $col_heading_expertname = "Expert Name";
   $col_heading_expertname = "<a href=\"experts_list_manager.cgi\">Expert Name</a>" if ($sortby ne '');
my $col_heading_lastupdated = "Last Updated";
   $col_heading_lastupdated = "<a href=\"experts_list_manager.cgi?sortby=lastupdated\">Last Updated</a>" if ($sortby ne 'lastupdated');
my $col_heading_active = "Active?";
   $col_heading_active = "<a href=\"experts_list_manager.cgi?sortby=active\">Active?</a>" if ($sortby ne 'active');
...
<TR bgcolor="#ebebeb">
	<td><strong>#</strong></td>
	<td><strong>$col_heading_expertname /Title</strong></td>
...
my $counter = 1;
	while (my @arr = $sth->fetchrow) {
		my ($expert_id, $expert_name, $expert_description, $expert_title, $expert_photo, $expert_link, $last_updated, $active, $expert_pd_session) = @arr;

		my $bgcolor="";
  			$bgcolor="BGCOLOR=\"#FFFFCC\"" if ($show_record eq $expert_id);
		## TRANSLATE CURLY QUOTES TO HTML ENTITIES
		$expert_name = &cleanaccents2html($expert_name);
		$last_updated = &convert_timestamp_2pretty_w_date($last_updated);
print<<EOM;
<TR $bgcolor>
	<td valign="top"><a name="$expert_id"></a>$counter</td>
	<td valign="top"><A HREF=\"experts_list_manager.cgi?location=add_item&show_record=$expert_id\" TITLE="Click to edit this $item_label">$expert_name</a><br>$expert_title</td>
	<td valign="top"><strong>$expert_pd_session</strong><br>$expert_description</td>
	<td valign="top"><img src="$expert_photo" height="102"></td>
	<td valign="top"><a href="$expert_link">$expert_link</a></td>
