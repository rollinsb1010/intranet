...
sub cleanthisfordb {
	...
	if ($clean_for_xss eq 'remove-xss') {
		...
		|| ($cleanitem =~ '\<embed') 
		|| ($cleanitem =~ 'embed\>') 
		|| (($cleanitem =~ '\<') && ($cleanitem =~ 'javascript'))
			) {
...
sub send_email {
	...
	my $message_content = $_[1];
	my $message_to = $_[2];
	   $message_to = 'webmaster@sedl.org' if ($message_to eq '');
	my $message_from = $_[3];
	   $message_from = 'webmaster@sedl.org' if ($message_from eq '');
...
sub printform_sedl_unit_menu {
	...
	my @item_value = ("", 
		...
		"EXECUTIVE OFFICE", "Improving School Performance (ISP)", 
		"IMPROVING SCHOOL PERFORMANCE (ISP) REL-SE", 
		"IMPROVING SCHOOL PERFORMANCE (ISP) SECC", 
		"IMPROVING SCHOOL PERFORMANCE (ISP) TXCC", 
	...
	my @item_label = ("(select one)", 
		...
		"DISABILITY RESEARCH TO PRACTICE (DRP)", 
		"EXECUTIVE OFFICE", 
		"Improving School Performance (ISP)", 
		"IMPROVING SCHOOL PERFORMANCE (ISP) REL-SE", 
		"IMPROVING SCHOOL PERFORMANCE (ISP) SECC", 
		"IMPROVING SCHOOL PERFORMANCE (ISP) TXCC", 
		"RESEARCH AND EVALUATION (R&E)");
	...
	if ($mixed_case eq 'no') {
		...
		$previous_selection = "COMMUNICATIONS" if ($previous_selection eq 'COM');
		$previous_selection = "IMPROVING SCHOOL PERFORMANCE (ISP) REL-SE" if ($previous_selection eq 'ISP-REL-SE');
		...
		$previous_selection = "REL SE" if ($previous_selection eq 'REL-SE');
		$previous_selection = "REL SW" if ($previous_selection eq 'REL-SW');
	} else {
		$previous_selection = "Administrative Services" if ($previous_selection eq 'AD');
		$previous_selection = "Afterschool, Family, and Community (AFC)" if ($previous_selection eq 'AFC');
		$previous_selection = "Communications" if ($previous_selection eq 'COM');
...
##########################################################################
### START: SUBROUTINE printform_sedl_areas_of_expertise_news
##########################################################################
#sub printform_sedl_areas_of_expertise_news {
#	my $field_name = $_[0];
#	my $previous_selection = $_[1];
#	my $hide_general_news_option = $_[2];
#	
...
#	print "</select>\n";
##########################################################################
#} # END: SUBROUTINE printform_sedl_areas_of_expertise_news
##########################################################################
