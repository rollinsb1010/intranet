...
EOM
	...
	my $command = "select * from staff_profiles where 
					firstname NOT LIKE '' 
...
if ($debug eq '1') {
	print "MATCHES: $num_matches";
		...
		while (my @arr = $sth->fetchrow) {
			my ($fm_record_id, $firstname, $middleinitial, $lastname, $jobtitle, $phone, $userid, $email, $phoneext, $birthmonth, $birthday, $timesheetname, $department_abbrev, $responsibilities, $experience, $mgmtcouncil, $lastupdated, $lastupdated_by, $room_number, $start_date, $adjusted_start_date, $stafflistsorting, $supervised_by, $automated_sentence, $photo_permissions, $show_birthday, $external_publications, $strong_pwd, $empl_type, $degree) = @arr;
...
my @departmentlistwithlink = (
	...
	"<strong>Disability Research to Practice (DRP) program</strong><br>
		Projects under this program include:
		<ul>
		<li><a href=\"http://www.ncddr.org\">National Center for the Dissemination of Disability Research</A> (NCDDR)</li>
		<li><a href=\"http://autism.sedl.org/\">Vocational Rehabilitation Service Models for Individuals with Autism Spectrum Disorders</a> (RUSH)</li>
...
<tr> 
	...
	<td valign="top" style="background-color: $bgcolor_alternating" class="noWrap">$phonenumber</td></tr>
EOM
...
<tr> 
	<td valign="top" style="background-color: $bgcolor_alternating"><a href="/pubs/catalog/authors/$userid.html">$boldingtag $lastname, $firstname$boldingtagclose</a> </td>
	<td valign="top" style="background-color: $bgcolor_alternating">$jobtitle</td>
	<td valign="top" style="background-color: $bgcolor_alternating" class="noWrap">$phonenumber</td>
</tr>
EOM
...
print STAFFPROFILEPAGE "$pd_sessions_taught\n" if $pd_sessions_taught ne '';
	...
	my $command = "select isactive, onlineid, title, title2, price, datepub, lochtml, locpdf, locdatabase
					from sedlcatalog 
	...
	my $count = "1";
		while (my @arr = $sth->fetchrow) {
			...
			my ($isactive, $onlineid, $title, $title2, $price, $datepub, $lochtml, $locpdf, $locdatabase) = @arr;
			...
			my $visual_clue_availablehow = "";
				...
				if ($isactive =~ 'n-profilepub') {
					...
					if (($title =~ 'SEDL Letter') || ($title =~ 'SEDLetter')) {
						...
						print STAFFPROFILEPAGE "<li><a href=\"/pubs/catalog/items/$onlineid_label\.html\" title=\"$this_title_metadata\">$title</A>";
						print STAFFPROFILEPAGE ": $title2" if $title2 ne '';
						print STAFFPROFILEPAGE " ($datepub)" if $datepub ne '';
						print STAFFPROFILEPAGE "</li>\n";
					}
...
print STAFFPROFILEPAGE "<p></p>\n$external_publications\n<p></p>" if $external_publications ne '';
	...
	if ($debug eq '1') {
		...
		print " <span style=\"color:#339900;\">....(<a href=\"/staff/personnel/staffprofiles.cgi?intranetonly=yes&showuserid=$userid\"><span style=\"color:#339900;\">intranet</span></A>)</span>\n";
	}
} ## END: LOOP THAT PRINTS ONE LINE FOR EACH STAFF ENTRY
...
if ($showuserid ne 'all') {


my $command = "select * from staff_profiles where userid like '$showuserid' order by lastname, firstname";
...
#	}
	if ($room_number ne '') {
print<<EOM;
<br>Room #: $room_number <a href="http://www.sedl.org/staff/planning/hq_floorplans/sedl_hq_floors_all.pdf">floor map</a>
EOM
	}
print<<EOM;
<br><br>
...
<a href="/staff/personnel/staffprofiles-print.cgi?intranetonly=yes&showuserid=all&list_view=birthday">Print Version</a>
</div>

<h1>SEDL Staff Listed by:</H1>
<a href="staffprofiles.cgi?intranetonly=yes&list_view=firstname&showuserid=all">First Name</A>, 
<a href="staffprofiles.cgi?intranetonly=yes&list_view=lastname&showuserid=all">Last Name</A>, 
<a href=\"staffprofiles.cgi?intranetonly=yes&list_view=deptname&showuserid=all\">Program/Department</A>, or Birthday
EOM
...
while ($loopingmonths < $numberofmonths) {
	...
	print "<a href=\"/staff/personnel/staffprofiles.cgi?intranetonly=yes&showuserid=$userid\" TITLE=\"$responsibilities\">$firstname $lastname</A></TD>";
	print "</TR>";
...
$heading_firstname = "<a href=\"staffprofiles.cgi?intranetonly=yes&list_view=firstname&showuserid=all\">First</A>" if ($list_view ne 'firstname');
$heading_lastname = "<a href=\"staffprofiles.cgi?intranetonly=yes&list_view=lastname&showuserid=all\">Last</A>" if ($list_view ne 'lastname');
$heading_department = "<a href=\"staffprofiles.cgi?intranetonly=yes&list_view=deptname&showuserid=all\">Program/Department</A>" if ($list_view ne 'deptname');
$heading_start_date = "<a href=\"staffprofiles.cgi?intranetonly=yes&list_view=startdatename&showuserid=all\">Start Date</A>" if ($list_view ne 'startdatename');
...
<h1>SEDL Staff Directory ($num_matches members)</H1>

<p>
You may view the staff list sorted by <a href=\"staffprofiles.cgi?intranetonly=yes&amp;showuserid=all&amp;list_view=birthday\">Birthday</a> 
...
#			}
	print "<TR style=\"background-color: $this_bgcolor\"><TD valign=\"top\" NOWRAP VALIGN=\"TOP\"><a href=\"/staff/personnel/staffprofiles.cgi?intranetonly=yes&showuserid=$userid\" onmouseover=\"Tip('$responsibilities', WIDTH, 600)\" onmouseout=\"UnTip()\">$firstname $lastname</A></TD><TD valign=\"top\" ALIGN=CENTER>";
		} else {
	print "<TR style=\"background-color: $this_bgcolor\"><TD valign=\"top\" NOWRAP VALIGN=\"TOP\"><a href=\"/staff/personnel/staffprofiles.cgi?intranetonly=yes&showuserid=$userid\">$lastname, $firstname</A></TD><TD valign=\"top\" ALIGN=CENTER>";
		}
	if ((lc($photo_permissions) eq 'on file') && ($staff_image_onfile{$userid} eq "yes")) {
...
print<<EOM;
	...
	<td VALIGN=\"TOP\" align=\"center\">$room_number</td>
...
<TR><TD ALIGN=CENTER valign="top"><IMG SRC="/staff/images/staffphotos/ceo/ehindsman.jpg"><BR>
	...
	<TD ALIGN=CENTER valign="top"><IMG SRC="/staff/images/staffphotos/ceo/wes-intranet1.jpg"><BR>
		<a href="/staff/personnel/staffprofiles.cgi?intranetonly=yes&showuserid=whoover">Wes Hoover</A><BR>
		(1996 - current)</TD></TR>
</TABLE>
