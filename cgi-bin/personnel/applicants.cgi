...
if ($location eq 'process_delete_item') {
	...
	my $this_user_name_full = "";
		...
		my $command = "select application_submission_status, name_f, name_l
		...
		my $num_matches_pubs = $sth->rows;
			while (my @arr = $sth->fetchrow) {
			my ($application_submission_status, $this_user_name_f, $this_user_name_l) = @arr;
				$this_user_name_full = "$this_user_name_f $this_user_name_l";
				if ($application_submission_status =~ 'yes') {
...
#			$location = "menu";
			...
			if ($reopen_for_editing eq 'yes') {
				 $command_update_user = "UPDATE $mysql_db_table_name 
										SET applyfor_position='$new_applyfor_position', name_l='$new_name_l', name_f='$new_name_f', hiring_supervisor='$new_hiring_supervisor', access_code='$new_access_code', position_exempt='$new_position_exempt', datestamp_completed = '$timestamp'
										WHERE record_id='$show_record'";
			}
...
my $num_matches_applicants = $sth->rows;
	...
	while (my @arr = $sth->fetchrow) {
		my ($record_id, $access_code, $form_complete, $application_submission_status, $email, $applyfor_position, $datestamp_created, $datestamp_completed, 
...
my $command = "select name_f, name_m, name_l
	...
	$command .= " order by application_submission_status DESC, applyfor_position, datestamp_completed DESC" if ($sortby eq 'completed');
...
my $command = "select record_id, applyfor_position, access_code, application_submission_status, datestamp_created, datestamp_completed, name_f, name_m, name_l, email, phone_day, phone_evening, hiring_supervisor, datestamp_updated
				from $mysql_db_table_name where access_code NOT LIKE 'review2007'";
	$command .= " AND hiring_supervisor = '$new_hiring_supervisor'" if ($new_hiring_supervisor ne '');
	$command .= " AND hiring_supervisor = '$cookie_ss_staff_id'" if ($logonuser_is_hr_representative ne 'yes');
	$command .= " AND datestamp_created > $show_datestamps_after" if ($show_time_period ne '');
...
my %applicants_bymanager;
	while (my @arr = $sth->fetchrow) {
		my ($record_id, $applyfor_position, $access_code, $application_submission_status, $datestamp_created, $datestamp_completed, $name_f, $name_m, $name_l, $email, $phone_day, $phone_evening, $hiring_supervisor, $datestamp_updated) = @arr;
		...
		if ($application_submission_status eq "yes") {
			$application_submission_status = "<font color=green>completed $datestamp_completed</font><br><A HREF=\"applicants.cgi?location=view_application&amp;show_record=$record_id\">view application</A>";
		} elsif (($phone_day eq '') && ($phone_evening eq '')) {
			$application_submission_status = "<font color=red>incomplete</font>";
		} else {
			$application_submission_status = "<font color=orange>underway</font><br><A HREF=\"applicants.cgi?location=view_application&amp;show_record=$record_id\">view application</A>";
		}
		if ($email ne '') {
			if (length($email) > 35) {
				$email =~ s/\@/ \@/gi;
...
print<<EOM;
	...
	<td valign="top">$application_submission_status
EOM
	if (($application_submission_status !~ 'completed') && ($logonuser_is_hr_representative eq 'yes')) {
print<<EOM;
