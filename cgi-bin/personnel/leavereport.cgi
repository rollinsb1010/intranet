...
if (($cookie_ss_staff_id eq 'lforador') || ($cookie_ss_staff_id eq 'mshuss') || ($cookie_ss_staff_id eq 'blitke') || ($cookie_ss_staff_id eq 'sferguso')) {
	print "<P class=\"info\"><strong>(Visible only to Matt, Stuart, Lori, and Brian)</strong> 
...
EOM
	...
	###########################################################################
	if (($cookie_ss_staff_id =~ 'blitke') || ($cookie_ss_staff_id =~ 'sferguso')) {
...
<p class="alert">WARNING: If you click a name below, their leave report records will be DELETED from the database.  Only do that for former staff whose records are no longer needed.</p>
EOM
		######################################
...
# $cookie_ss_staff_id = "blitke"; ## FOR TESTING ONLY, COMMENT OUT WHEN LIVE
	...
	my $command = "select * from staffleavereport 
		...
		my $command_get_supervised_staff = "select timesheetname from staff_profiles where
											(
											(immediate_supervisor_sims_user_ID LIKE '%$cookie_ss_staff_id%')
											OR (bgt_auth_primary_sims_user_ID LIKE '%$cookie_ss_staff_id%')
											OR (time_leave_admin_sims_user_ID LIKE '%$cookie_ss_staff_id%')
											)";
		my $dsn = "DBI:mysql:database=intranet;host=localhost";
		my $dbh = DBI->connect($dsn, "intranetuser", "limited");
		...
		$sth->execute;
		my $num_matches = $sth->rows;
#			print "<p class=\"info\">COMMAND TO GET LIST OF STAFF I SUPERVISE: $command_get_supervised_staff<BR><BR>MATCHES: $num_matches</p>";
			while (my @arr = $sth->fetchrow) {
				my ($timesheetname_of_person_supervised) = @arr;
	   			$command .= " OR (timesheetname like '%$timesheetname_of_person_supervised%') ";
#	   			print "<p>I SUPERVISE: $timesheetname_of_person_supervised</p>";
			} # END WHILE LOOP
		#################################################################################################
