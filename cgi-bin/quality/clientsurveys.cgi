...
EOM
print "<br>		- <a href=\"clientsurveys.cgi?location=maintenance\">maintenance menu</a>" if (($cookie_ss_staff_id eq 'brollins') || ($cookie_ss_staff_id eq 'blitke'));
print<<EOM;
	</TD></TR>
</TABLE>
...
for prompting site visitors to give us their email address 
before accessing an online SEDL resource. Although entering the email address is not required to access the resource, 
if the user enters their email address, it is saved to a database that will automatically send that 
person an email after one week has passed. The email will include a link back to our site where the 
...
unit for a previous REL contract. This database allows you to see 
	...
	<ul>
	<li>a list of email addresses volunteered by viewers of each publication,</li>
...
print<<EOM;
<table border="1" cellpadding="8" cellspacing="0" width="100%" style="background-color:#ffffff;">
...
my $quarter = "";
    foreach $quarter (sort (keys(%summary_byquarter))) {
		...
		my $bar_color = "purple";
		   ...
		   $bar_color = "green" if (($t_year eq 2005) || ($t_year eq 2009) || ($t_year eq 2013));
		   $bar_color = "red" if (($t_year eq 2002) || ($t_year eq 2006) || ($t_year eq 2010) || ($t_year eq 2014));
...
print<<EOM;
		<tr style="text-align:center;">
			<td colspan="3" style="border:1px solid #999999;">2002</td>
			<td colspan="4" style="border:1px solid #999999;">2003</td>
			<td colspan="4" style="border:1px solid #999999;">2004</td>
			<td colspan="4" style="border:1px solid #999999;">2005</td>
			<td colspan="4" style="border:1px solid #999999;">2006</td>
			<td colspan="4" style="border:1px solid #999999;">2007</td>
			<td colspan="4" style="border:1px solid #999999;">2008</td>
			<td colspan="4" style="border:1px solid #999999;">2009</td>
			<td colspan="4" style="border:1px solid #999999;">2010</td>
			<td colspan="4" style="border:1px solid #999999;">2011</td>
			<td colspan="4" style="border:1px solid #999999;">2012</td>
			<td colspan="4" style="border:1px solid #999999;">2013</td>
			<td colspan="4" style="border:1px solid #999999;">2014</td>
		</tr>
		</table>
		<br>
...
<tr><td valign="top" colspan="2">
		...
		<div style="width:100%; height:400px; overflow: scroll; background-color:#ffffff;border:0px;padding:0px;margin-top:-4px;">
			<table border="1" cellpadding="2" cellspacing="0">
			<tr style="background-color:#ebebeb;">
				<td>Product Title</td>
...
print<<EOM;
<p style="padding:0px;margin:2px 0 5px 0;"><strong><a href="clientsurveys.cgi?location=listsurveys&amp;showdocid=$this_doc_id&amp;show_responses_only=yes">$summary_bydocid{$product_id}</a></strong></p>
EOM
		if ($summary_bydocid{$product_id} > 0) {
print<<EOM;
...
$sth->execute;
	while (my @arr = $sth->fetchrow) {
		...
		if ($assignments_by_staff_id{$userid} > 0) {
			...
			my $s = "s";
			   $s = "" if ($assignments_by_staff_id{$userid} == 1);
			$pulldownmenu_for_staff_assignments .= ">$firstname $lastname (assigned to $assignments_by_staff_id{$userid} resource$s)</option>\n";
		} # END IF
...
print<<EOM;
	...
	<td valign="top" width="50%">
		<h2 style="margin-top:0px;">Search by Client email</h2>
		<p>You can enter a full or partial email address to see if that person has been surveyed.</p>
		<form action="clientsurveys.cgi" method="GET">
		<div style="margin-left:25px;">
		<input type="text" size="25" name="search_email" id="search_email" value="">

		<input type="hidden" name="location" value="listsurveys"><br>
		<input type="submit" value="Find this email">
		</div>
		</form>
	</td>
</tr>
...
<span style="color:#996600;"><em><strong>Note:</strong> We told the Web site visitors their email addresses would only be used to contact 
...
<TR><TD VALIGN="TOP"><IMG SRC="images/surveybounced.gif" ALT="Status: Bounced" class="noBorder"></TD><TD>User email bounced/invalid</TD></TR>
</TABLE>
EOM
...
print "<p class=\"info\">Displaying $num_matches surveys matching your search criteria.</p>";
	...
	my %email_domains; # HASH TO REMEMBER HOW MANY USERS FROM WHICH DOMAINS ARE IN THE LIST
	...
	while (my @arr = $sth->fetchrow) {
		...
		my ($trash, $this_domain) = split(/\@/,$email);	
		$email_domains{$this_domain}++;
		$count++;
	} # END DB QUERY LOOP
...
<h2>Summary of Survey Participants by Domain</h2>
<table border="1" cellpadding="4" cellspacing="0">
<tr><td><strong>Domain</strong></td><td><strong>How many in the list?</strong></td></tr>
EOM


    foreach my $email_domain (sort {$email_domains {$b} <=> $email_domains {$a}} (keys(%email_domains))) {
			print "<tr><td>$email_domain</td><td>$email_domains{$email_domain}</td></tr>";
		}
	print "</table>";
...
#print "<p class=\"info\">COMMAND: $command<br><br>MATCHES: $num_matches</p>";
	while (my @arr = $sth->fetchrow) {
		my ($recordid, $surveysent, $surveysenttwice, $surveyreceived, $email, $date, $documenturl, $documentid, $documentgroup, 
			...
			$ipnum, $browser, $request_followup, $permission_use_comments, $name, $title, $staff_comments, $staff_comments_by, $staff_comments_date, $state) = @arr;
...
#################################################################################
## START: LOCATION = LISTDOCUMENTSBYTITLE
#################################################################################
...
#print "<p class=\"info\">MATCHES: $num_matches</p>";
	while (my @arr = $sth->fetchrow) {
		my ($recordid, $surveysent, $surveysenttwice, $surveyreceived, $email, $date, $documenturl, $documentid, $documentgroup, $q1a, $q1b, $q1c, $q1d, $q2a, $q2b, $q3a, $q3b, $q3c, $q3d, $q3e, $q4a, $q5a, $q6a, $q7a, $q8a, $q9a, $q10a, $ipnum, $browser, $request_followup, $permission_use_comments, $name, $title, $staff_comments, $staff_comments_by, $staff_comments_date, $state) = @arr;
...
$state = "NOT INCLUDED BY USER" if ($state eq '');
print<<EOM;
<p>
User Name: $name<br>
User title: $title<br>
...
<TR><TD>
<h2 ALIGN="CENTER" style="margin-top:0px;">THIS AREA FOR SEDL USE ONLY</h2>
...
<TR><TD VALIGN="TOP">
		...
		This page is used by Communications office staff to flag bounced emails<br> and fix emails entered incorrectly.</p>
		<P>
...
<p>
<label for="badaddress1">Simply type in the email address below to mark its records in the database as an invalid email address.</label>
</p>
	<div style="margin-left:25px;">
	...
	<input type="submit" value="Click to mark this as a bounced email address">
	</form>
	</div>



<h2>Fix an email and re-send survey</h2>
<p>
Simply type in the two email addresses below to change the email address and re-send the survey invitation.
</p>
<form method="POST" action="clientsurveys.cgi">
	<div style="margin-left:25px;">
	<TABLE>
	...
	<input type="submit" value="Click to fix this email">
	</form>
	</div>
